...
bool my_data[10] = {1, 1, 1, 0, 1, 1, 1, 1, 1, 1};
...
void W25Q64_CS_LOW()  { HAL_GPIO_WritePin(CS_GPIO_Port, CS_Pin, GPIO_PIN_RESET); }
void W25Q64_CS_HIGH() { HAL_GPIO_WritePin(CS_GPIO_Port, CS_Pin, GPIO_PIN_SET); }
...
int main(void)
{
  ...
  for (int i = 0; i < 10; i++) {
      write_buffer[i] = my_data[i] ? 1 : 0;
  }

  // Write to flash memory at address 0x000000
  W25Q64_PageProgram(0x000000, write_buffer, 10);

  // Optional delay to wait for the write to complete
  HAL_Delay(100);

  // Read back the 10 bytes
  W25Q64_ReadData(0x000000, read_buffer, 10);
